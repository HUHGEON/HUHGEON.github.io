...
updated: "2026-06-09 13:17:30 +0900"
tags:
    - 개발
    - 방법론
    - 테스트
---
# [TDD] 이번 프로젝트에 TDD를 도입한 이유 — 테스트를 먼저 쓰니 AI가 알아서 검증한다

> ⚠️ 상태: 이번 프로젝트는 도메인 순수 로직부터 TDD를 기본으로 가져간다. 아래 사례(권한 정책·비속어 필터)는 그중 일부다. I/O 경계는 통합 테스트로 분리(§7).
...
TDD(Test-Driven Development, 테스트 주도 개발) 는 [Martin Fowler의 정의](https://martinfowler.com/bliki/TestDrivenDevelopment.html)에 따르면 "테스트를 작성함으로써 소프트웨어 개발을 이끄는 기법"이다. Kent Beck이 1990년대 후반 익스트림 프로그래밍(XP)의 일부로 정립했다.
...
> 💡 Fowler는 "TDD를 망치는 가장 흔한 방법은 세 번째 단계(Refactor)를 빼먹는 것"이라고 경고한다. Kent Beck의 표현으로는 "Make it run, then make it right" — 일단 통과시키고, 그다음에 바르게 만든다.
...
| 위험 | 통과만 보고 안심 | Refactor 빼먹으면 코드가 지저분 |
...
매칭 위치(span)는 **원본 길이 기준**으로 잡아뒀는데, 변환 후 문자열이 더 길어지면서 그 인덱스가 원본 범위를 벗어났다 → `StringIndexOutOfBoundsException`.
...
- Kent Beck, 『Test-Driven Development: By Example』 (2002) — 도서
