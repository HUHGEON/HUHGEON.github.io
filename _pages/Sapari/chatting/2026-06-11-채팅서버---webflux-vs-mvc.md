...
date: "2026-06-11 10:48"
updated: "2026-06-11 11:03:42 +0900"
tags:
    - 개발
    - 채팅
    - WebSocket
    ...
    - Spring
---
# [WebFlux] 라이브 커머스 채팅 서버 — WebSocket을 어떤 스택으로 받을까 (MVC vs WebFlux)
...
진짜 고민은 그다음이었습니다. **"이 WebSocket 연결을 어떤 웹 스택으로 받을 것인가."** 우리 서비스의 일반 REST API는 Spring MVC로 돌아가는데, 채팅도 같은 스택에 얹을지 아니면 WebFlux로 분리할지가 문제였습니다. 이 글은 그 갈림길에서 **실시간 채팅 부분만 WebFlux로 떼어낸 이유와, 그 대가로 무엇을 떠안았는지**에 대한 기록입니다.
...
> 💡 한 가지 분명히 해 둘 것은, **MVC를 버린 게 아니라는** 점입니다. 라이브 CRUD·강퇴·채팅 이력 조회 같은 평범한 REST는 기존 api-app(MVC)에 그대로 뒀고, **실시간 WebSocket 채팅만** 별도 앱(streaming-app)으로 떼어 WebFlux로 받았습니다. 트래픽 성격이 다른 둘을 갈라 놔야 **각각 독립적으로 스케일(k8s HPA)** 할 수 있기 때문입니다. "어디에 WebFlux를 쓸지 고르는 것"도 하나의 설계 결정이었습니다.

## 5. MVC vs WebFlux

정리하면 이렇습니다.

| 항목 | Spring MVC (REST 쪽 유지) | Spring WebFlux (실시간 채팅 쪽) |
|---|---|---|
...
## 7. 대가 ②: "그럼 저장은? Redis는?" 줄줄이 딸려온 결정들

처음엔 단순하게 봤습니다. WebSocket 핸들러만 WebFlux로 바꾸면 되겠거니. 그런데 막상 손을 대니 질문이 꼬리를 물었습니다. **그럼 메시지 저장은? Redis 조회는? JWT 검증은?** 스택을 고른 결정 하나가 그 아래 모든 선택을 다시 묻게 만들었습니다.

이유는 하나입니다. 이벤트루프는 적은 스레드로 돌기 때문에, **그중 하나라도 블로킹 호출에 멈추면 그 스레드가 맡던 수많은 연결이 통째로 대기**합니다. MVC라면 스레드 하나 잠깐 멈추는 정도로 끝날 일이, WebFlux에선 전체 처리량을 깎아먹습니다. 그러니 위의 질문 하나하나에 **"블로킹 없이 어떻게?"**로 답을 찾아야 했습니다.
...
다만 분명히 해 둘 게 있습니다. 이 "블로킹 금지"는 **시스템 전체의 규칙이 아닙니다.** 강퇴·채팅 이력 조회 같은 **REST는 여전히 api-app에서 블로킹 MVC로** 돌아갑니다 — 오히려 그 유스케이스들은 일부러 `void`·일반 결과 객체(블로킹) 타입으로 뒀습니다. MVC에서 `Mono`를 받으면 결국 `.block()`을 호출하게 되는데, 그게 이점도 없이 복잡도만 키우기 때문입니다. 즉 **블로킹 금지는 이벤트루프 위에서 도는 WS 채팅 전송 쪽(streaming-app)에만** 걸린 제약입니다.
...
그래서 제약을 받는 건 streaming-app 한쪽이지만, 그 한쪽에서는 앞서 꼬리를 물던 질문마다 답이 하나씩 필요했습니다. 줄줄이 딸려온 결정들은 이렇습니다.
...
흥미로운 건 **같은 `chat_messages` 컬렉션을 두 방식으로 다루도록 설계했다**는 점입니다. 송신 경로(streaming-app)는 reactive 드라이버로 **쓰고**, 이력 조회(api-app)는 평범한 블로킹 드라이버로 **읽습니다**(각 앱의 `build.gradle`에 reactive·blocking Mongo 스타터가 따로 물려 있습니다). 저장소가 같아도, 그 위에 어떤 드라이버를 얹느냐는 쪽마다 다릅니다.
...
> ✅ 핵심은 이겁니다 — WebFlux는 "선언하면 빨라지는 옵션"이 아니라, **그 위에서 도는 코드 전부가 '블로킹 금지'를 받아들여야** 효율이 나오는 구조입니다. 그래서 우리는 채팅 전체를 reactive로 갈아엎은 게 아니라, **블로킹이 치명적인 전송 쪽만 reactive로 떼어 격리하고** 나머지는 익숙한 MVC에 남겨 둔 뒤, 그 경계를 ArchUnit으로 못 박았습니다.
...
정리하면 이렇습니다. 채팅은 양방향·실시간이라 전송은 **WebSocket**이 정석이고, 그 연결은 **오래 살아 있다**는 특성이 있습니다. 그래서 강퇴·이력 같은 평범한 REST는 MVC에 그대로 두고, **연결이 폭증하고 메시지가 몰리는 실시간 채팅 전송만** 떼어내 WebFlux로 받았습니다. 그 전송 쪽에 한해 **STOMP의 편의를 포기**하고 순수 WebSocket으로 프로토콜을 직접 설계했으며(그 덕에 신뢰 경계는 직접 통제), **"블로킹 금지"를 받아들여** 서블릿 의존을 분리하고 reactive MongoDB/Redis 드라이버를 물렸으며 메시지당 DB조회를 없애도록 설계했고, reactive와 blocking 유스케이스가 한 모듈에 공존하는 경계는 **ArchUnit 규칙**으로 가르도록 했습니다.
